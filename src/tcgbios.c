...
#define TPM_PPI_FLAGS (TPM_PPI_FUNC_ACTION_REBOOT | \
...
tpm_ppi_init(void)
{
    ...
    tpm_ppi_config = romfile_loadfile("etc/tpm/ppi", &size);
    if (!tpm_ppi_config)
        return;
...
static int
tpm12_process_cfg(tpm_ppi_code msgCode, int verbose, u8 *nextStep)
{
    int ret = 0;
